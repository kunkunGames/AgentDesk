...
fn check_qwen_runtime_artifacts(configured: bool) -> Check {
    ...
    let guidance = "Qwen uses local assets like ~/.qwen/extensions, ~/.qwen/skills, <workspace>/.qwen/PROJECT_SUMMARY.md, and <workspace>/.qwen/.env as is. In headless environments, check if the project .qwen/.env takes precedence.";
    if configured {
        Check::warn(
            "provider_qwen_runtime",
            CheckGroup::ProviderRuntime,
...
fn check_config_audit(snapshot: &HealthSnapshot) -> Check {
    ...
    if status == "ok" && warnings_count == 0 {
        ...
        Check::warn(
            ...
            "Config Audit",
            detail.clone(),
            "Check the drift summary for agentdesk.yaml, legacy role map, or bot settings. Raw source paths are not exposed in the public health payload.",
        )
        .with_subsystem("config_audit")
        .with_path(health_detail_endpoint(&snapshot.base))
        .with_expected_actual("config audit ok", detail)
...
fn check_credential_permissions(cfg: &config::Config) -> Check {
    ...
    if risks.is_empty() {
        ...
        Check::warn(
            ...
            "Credential and config file contents were not read or logged; only permission and owner metadata were checked.",
        )
        .with_subsystem("security")
...
fn check_runtime_path() -> Check {
    ...
    match crate::services::platform::merged_runtime_path() {
        ...
        None => Check::fail(
            ...
            "Could not read the login shell PATH. Compare the service environment PATH with the shell PATH.",
        )
        ...
        None => {
            ...
            let (status, severity, guidance) = if error.contains("(401)") || error.contains("(403)")
            {
                ...
                (
                    ...
                    Severity::Critical,
                    "To send tokens to a non-loopback URL, you must explicitly use --allow-remote.",
                )
            } else {
                (
                    "unreachable",
...
fn check_data_dir(cfg: &config::Config) -> Check {
    if cfg.data.dir.exists() && cfg.data.dir.is_dir() {
        ...
        Check::fail(
            ...
            "You can create the data directory and DB by running agentdesk doctor --fix.",
        )
        .with_path(cfg.data.dir.display().to_string())
...
fn check_service_manager() -> Check {
    ...
    if dcserver::is_launchd_job_loaded(&label) {
        ...
        Check::warn(
            ...
            "If running via launchd, check the plist load status. You can ignore this in manual run environments.",
        )
        .with_expected_actual(
            "launchd job loaded or tmux fallback active",
...
fn check_file_descriptor_headroom() -> Check {
    ...
    if launchd_limit.is_none() {
        return Check::warn(
            ...
            "Could not read the macOS launchd limit, so tmux/dcserver FD usage cannot be compared against it.",
        )
...
fn check_service_manager() -> Check {
    ...
    if active {
        ...
        Check::warn(
            ...
            "If you plan to run as a service, verify if the systemd user service is registered.",
        )
        .with_expected_actual(
        ...
        .build()
    {
        ...
        Err(error) => {
            return Check::fail(
                ...
                "Failed to build the async runtime for Postgres connection verification.",
            )
            .with_expected_actual(
...
fn check_stale_zero_byte_db_files(cfg: &config::Config) -> Check {
    let Some(runtime_root) = dcserver::agentdesk_runtime_root() else {
        return Check::warn(
            ...
            "Verify the canonical DB path first, then remove zero-byte stale DB files from the root path.",
        )
        .with_expected_actual(
            "runtime root path resolvable",
            "runtime root path unresolved",
    ...
    Check::warn(
        ...
        format!(
            "The canonical DB is {}. Do not open guessed paths with sqlite3; use agentdesk doctor --fix to remove stale files if needed.",
            canonical_db_path.display()
        ),
    )
    .with_path(runtime_root.display().to_string())
    ...
    match dcserver::agentdesk_runtime_root() {
        ...
        Some(path) => match recursive_dir_size(&path) {
            ...
            Err(e) => Check::warn(
                ...
                "Check permissions for the runtime root.",
            )
            .with_path(path.display().to_string())
            .with_expected_actual(
