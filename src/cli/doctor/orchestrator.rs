...
fn check_qwen_runtime_artifacts(configured: bool) -> Check {
    ...
    let guidance = "Qwen uses local assets like ~/.qwen/extensions, ~/.qwen/skills, <workspace>/.qwen/PROJECT_SUMMARY.md, and <workspace>/.qwen/.env as-is. In a headless environment, also verify whether the project's .qwen/.env takes precedence.";
    if configured {
        Check::warn(
            "provider_qwen_runtime",
            CheckGroup::ProviderRuntime,
...
fn check_health_db_dashboard(snapshot: &HealthSnapshot) -> Check {
    let Some(body) = snapshot.body.as_ref() else {
        return Check::fail(
            ...
            "DB/dashboard state summary requires access to the dcserver health detail endpoint.",
        )
...
fn check_dispatch_outbox(snapshot: &HealthSnapshot) -> Check {
    let Some(body) = snapshot.body.as_ref() else {
        return Check::fail(
            ...
            "Dispatch Outbox",
            "health payload unavailable",
            "Failed to read dispatch outbox health.",
        )
...
fn check_config_audit(snapshot: &HealthSnapshot) -> Check {
    let Some(body) = snapshot.body.as_ref() else {
        return Check::fail(
            ...
            "Config Audit",
            "health payload unavailable",
            "Failed to read config audit report.",
        )
...
fn check_runtime_path() -> Check {
    ...
    match crate::services::platform::merged_runtime_path() {
        ...
        None => Check::fail(
            ...
            "Failed to read login shell PATH. Compare the service environment PATH with the shell PATH.",
        )
...
fn check_service_manager() -> Check {
    ...
    if dcserver::is_launchd_job_loaded(&label) {
        ...
        Check::warn(
            ...
            "If running with launchd, verify the plist load status. You can ignore this in a manual execution environment.",
        )
        .with_expected_actual(
            "launchd job loaded or tmux fallback active",
...
fn check_file_descriptor_headroom() -> Check {
    ...
    if launchd_limit.is_none() {
        return Check::warn(
            ...
            "Could not read macOS launchd limit, so tmux/dcserver FD usage could not be compared to the limit.",
        )
...
fn check_service_manager() -> Check {
    ...
    if active {
        ...
        Check::warn(
            ...
            "If planning to operate as a service, check if the systemd user service is registered.",
        )
        .with_expected_actual(
        ...
        .build()
    {
        ...
        Err(error) => {
            return Check::fail(
                ...
                "Failed to create an async runtime for postgres connection verification.",
            )
            .with_expected_actual(
...
fn check_stale_zero_byte_db_files(cfg: &config::Config) -> Check {
    let Some(runtime_root) = dcserver::agentdesk_runtime_root() else {
        return Check::warn(
            ...
            "Confirm the actual DB path first, then remove 0-byte stale DB files in the root path.",
        )
        .with_expected_actual(
            "runtime root path resolvable",
            "runtime root path unresolved",
    ...
    Check::warn(
        ...
        format!(
            "The actual DB is {}. Do not open sqlite3 with a guessed path. If necessary, clean up stale files with agentdesk doctor --fix.",
            canonical_db_path.display()
        ),
    )
    .with_path(runtime_root.display().to_string())
