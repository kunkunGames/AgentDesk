...
fn check_qwen_runtime_artifacts(configured: bool) -> Check {
    ...
    let guidance = "Qwen uses local assets such as ~/.qwen/extensions, ~/.qwen/skills, <workspace>/.qwen/PROJECT_SUMMARY.md, and <workspace>/.qwen/.env as-is. In a headless environment, also verify whether the project .qwen/.env takes precedence.";
    if configured {
        Check::warn(
            "provider_qwen_runtime",
            CheckGroup::ProviderRuntime,
...
fn check_config_audit(snapshot: &HealthSnapshot) -> Check {
    ...
    if status == "ok" && warnings_count == 0 {
        ...
        Check::warn(
            ...
            "Config Audit",
            detail.clone(),
            "Check the agentdesk.yaml/legacy role map/bot settings drift summary. The public health does not expose the raw source path.",
        )
        .with_subsystem("config_audit")
        .with_path(health_detail_endpoint(&snapshot.base))
        .with_expected_actual("config audit ok", detail)
...
fn check_credential_permissions(cfg: &config::Config) -> Check {
    ...
    if risks.is_empty() {
        ...
        Check::warn(
            ...
            "The credential/config file contents were not read or printed; only permissions/owner metadata were checked.",
        )
        .with_subsystem("security")
...
fn check_service_manager() -> Check {
    ...
    if dcserver::is_launchd_job_loaded(&label) {
        ...
        Check::warn(
            ...
            "Check the plist load status if running with launchd. If running manually, this can be safely ignored.",
        )
        .with_expected_actual(
            "launchd job loaded or tmux fallback active",
...
fn check_file_descriptor_headroom() -> Check {
    ...
    if launchd_limit.is_none() {
        return Check::warn(
            ...
            "macOS launchd limit could not be read, so tmux/dcserver FD usage could not be compared against the limit.",
        )
...
fn check_service_manager() -> Check {
    ...
    if active {
        ...
        Check::warn(
            ...
            "If planning to run as a service, verify that the systemd user service is registered.",
        )
        .with_expected_actual(
        ...
        .build()
    {
        ...
        Err(error) => {
            return Check::fail(
                ...
                "Failed to create async runtime for postgres connection validation.",
            )
            .with_expected_actual(
...
fn check_stale_zero_byte_db_files(cfg: &config::Config) -> Check {
    let Some(runtime_root) = dcserver::agentdesk_runtime_root() else {
        return Check::warn(
            ...
            "First check the actual DB path, then clean up 0-byte stale DB files in the root path.",
        )
        .with_expected_actual(
            "runtime root path resolvable",
            "runtime root path unresolved",
    ...
    Check::warn(
        ...
        format!(
            "The actual DB is {}. Do not open sqlite3 with a guessed path; if needed, run agentdesk doctor --fix to clean up stale files.",
            canonical_db_path.display()
        ),
    )
    .with_path(runtime_root.display().to_string())
    ...
    match dcserver::agentdesk_runtime_root() {
        ...
        Some(path) => match recursive_dir_size(&path) {
            ...
            Err(e) => Check::warn(
                ...
                "Verify the runtime root permissions.",
            )
            .with_path(path.display().to_string())
            .with_expected_actual(
