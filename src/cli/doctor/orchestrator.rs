...
fn check_qwen_runtime_artifacts(configured: bool) -> Check {
    ...
    let guidance = "Qwen uses local assets like ~/.qwen/extensions, ~/.qwen/skills, <workspace>/.qwen/PROJECT_SUMMARY.md, <workspace>/.qwen/.env as is. In a headless environment, also check whether the project .qwen/.env takes precedence.";
    if configured {
        Check::warn(
            "provider_qwen_runtime",
            CheckGroup::ProviderRuntime,
...
fn check_config_audit(snapshot: &HealthSnapshot) -> Check {
    ...
    if status == "ok" && warnings_count == 0 {
        ...
        Check::warn(
            ...
            "Config Audit",
            detail.clone(),
            "Check the agentdesk.yaml/legacy role map/bot settings drift summary. The public health does not expose raw source paths.",
        )
        .with_subsystem("config_audit")
        .with_path(health_detail_endpoint(&snapshot.base))
        .with_expected_actual("config audit ok", detail)
...
fn check_credential_permissions(cfg: &config::Config) -> Check {
    ...
    if risks.is_empty() {
        ...
        Check::warn(
            ...
            "The contents of credential/config files were not read or printed; only permissions/owner metadata were checked.",
        )
        .with_subsystem("security")
        ...
        None => {
            ...
            let (status, severity, guidance) = if error.contains("(401)") || error.contains("(403)")
            {
                ...
                (
                    ...
                    Severity::Critical,
                    "To send a token to a non-loopback URL, explicitly use --allow-remote.",
                )
            } else {
                (
                    "unreachable",
...
fn check_service_manager() -> Check {
    ...
    if dcserver::is_launchd_job_loaded(&label) {
        ...
        Check::warn(
            ...
            "If operating with launchd, check the plist load status. You can ignore this in a manual execution environment.",
        )
        .with_expected_actual(
            "launchd job loaded or tmux fallback active",
...
fn check_file_descriptor_headroom() -> Check {
    ...
    if launchd_limit.is_none() {
        return Check::warn(
            ...
            "Could not read the macOS launchd limit, so tmux/dcserver FD usage could not be compared to the limit.",
        )
