...
fn check_qwen_runtime_artifacts(configured: bool) -> Check {
    ...
    let guidance = "Qwen natively uses local assets like ~/.qwen/extensions, ~/.qwen/skills, <workspace>/.qwen/PROJECT_SUMMARY.md, and <workspace>/.qwen/.env. In headless environments, verify whether the project .qwen/.env takes precedence.";
    if configured {
        Check::warn(
            "provider_qwen_runtime",
            CheckGroup::ProviderRuntime,
...
fn check_config_audit(snapshot: &HealthSnapshot) -> Check {
    ...
    if status == "ok" && warnings_count == 0 {
        ...
        Check::warn(
            ...
            "Config Audit",
            detail.clone(),
            "Check the agentdesk.yaml/legacy role map/bot settings drift summary. The public health payload does not expose raw source paths.",
        )
        .with_subsystem("config_audit")
        .with_path(health_detail_endpoint(&snapshot.base))
        .with_expected_actual("config audit ok", detail)
        ...
        None => {
            ...
            let (status, severity, guidance) = if error.contains("(401)") || error.contains("(403)")
            {
                ...
                (
                    ...
                    Severity::Critical,
                    "To send a token to a non-loopback URL, you must explicitly use --allow-remote.",
                )
            } else {
                (
                    "unreachable",
...
fn check_service_manager() -> Check {
    ...
    if dcserver::is_launchd_job_loaded(&label) {
        ...
        Check::warn(
            ...
            "If operating with launchd, verify the plist load status. You can ignore this in a manual execution environment.",
        )
        .with_expected_actual(
            "launchd job loaded or tmux fallback active",
...
fn check_file_descriptor_headroom() -> Check {
    ...
    if launchd_limit.is_none() {
        return Check::warn(
            ...
            "Unable to read the macOS launchd limit, so the tmux/dcserver FD usage could not be compared against the limit.",
        )
...
fn check_service_manager() -> Check {
    ...
    if active {
        ...
        Check::warn(
            ...
            "If you plan to run this as a service, verify whether the systemd user service is registered.",
        )
        .with_expected_actual(
...
fn check_stale_zero_byte_db_files(cfg: &config::Config) -> Check {
    let Some(runtime_root) = dcserver::agentdesk_runtime_root() else {
        return Check::warn(
            ...
            "First verify the actual DB path, then clean up the 0-byte stale DB files in the root path.",
        )
        .with_expected_actual(
            "runtime root path resolvable",
            "runtime root path unresolved",
    ...
    Check::warn(
        ...
        format!(
            "The actual DB is {}. Do not open sqlite3 with the guessed path; if needed, use agentdesk doctor --fix to clean up stale files.",
            canonical_db_path.display()
        ),
    )
    .with_path(runtime_root.display().to_string())
