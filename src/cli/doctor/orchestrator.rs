...
fn check_credential_permissions(cfg: &config::Config) -> Check {
    ...
    if risks.is_empty() {
        ...
        Check::warn(
            ...
            "The contents of the credential/config file were not read or printed; only permission/owner metadata was checked.",
        )
        .with_subsystem("security")
        ...
        None => {
            ...
            let (status, severity, guidance) = if error.contains("(401)") || error.contains("(403)")
            {
                ...
                (
                    ...
                    Severity::Critical,
                    "To send a token to a non-loopback URL, use --allow-remote explicitly.",
                )
            } else {
                (
                    "unreachable",
...
fn check_service_manager() -> Check {
    ...
    if dcserver::is_launchd_job_loaded(&label) {
        ...
        Check::warn(
            ...
            "If operating with launchd, check the plist load status. If in a manual execution environment, this can be ignored.",
        )
        .with_expected_actual(
            "launchd job loaded or tmux fallback active",
...
fn check_file_descriptor_headroom() -> Check {
    ...
    if launchd_limit.is_none() {
        return Check::warn(
            ...
            "Failed to read the macOS launchd limit, so tmux/dcserver FD usage could not be compared against the limit.",
        )
...
fn check_service_manager() -> Check {
    ...
    if active {
        ...
        Check::warn(
            ...
            "If you plan to operate it as a service, check if the systemd user service is registered.",
        )
        .with_expected_actual(
...
fn check_stale_zero_byte_db_files(cfg: &config::Config) -> Check {
    ...
    Check::warn(
        ...
        format!(
            "The actual DB is {}. Do not open sqlite3 with a guessed path, and if necessary, use 'agentdesk doctor --fix' to clean up stale files.",
            canonical_db_path.display()
        ),
    )
    .with_path(runtime_root.display().to_string())
