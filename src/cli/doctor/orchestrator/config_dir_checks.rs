...
/// contract ("N unregistered items in folder: …") is unit-testable without a DB.
fn routine_script_registration_check(
    ...
    Check::warn(
        ...
        ROUTINE_SCRIPT_REGISTRATION_CHECK_NAME,
        format!(
            "{} unregistered items in folder: {}",
            unregistered.len(),
            unregistered.join(", ")
        ),
        "*.js in routines.dir is not attached to the routines table. Register with POST /api/routines or remove the file.",
    )
    .with_subsystem("routines")
    .with_path(dirs_display)
    .with_expected_actual(
...
pub(super) fn check_routine_script_registration(cfg: &config::Config) -> Check {
        ...
        .build()
    {
        ...
        Err(error) => {
            return Check::warn(
                ...
                "Failed to create async runtime for querying routines table.",
            )
            .with_subsystem("routines")
            .with_path(dirs_display)
            .with_expected_actual(
    ...
    match registered {
        ...
        Err(error) => Check::warn(
            ...
            CheckGroup::Core,
            ROUTINE_SCRIPT_REGISTRATION_CHECK_NAME,
            format!("{dirs_display} — routines table query failed"),
            "Could not determine unregistered scripts due to Postgres connection or routines table query failure.",
        )
        .with_subsystem("routines")
        .with_path(dirs_display)
...
pub(super) fn check_data_dir(cfg: &config::Config) -> Check {
    if cfg.data.dir.exists() && cfg.data.dir.is_dir() {
        ...
        Check::fail(
            ...
            "Run agentdesk doctor --fix to create the data directory and DB.",
        )
        .with_path(cfg.data.dir.display().to_string())
...
mod routine_script_registration_tests {
    ...
    fn unregistered_scripts_are_reported_as_one_warn_item() {
        ...
        assert_eq!(
            check.detail,
            "2 unregistered items in folder: dependency-update-watcher.js, nested/x.js"
        );
