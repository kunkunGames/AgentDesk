...
pub(super) fn check(cfg: &config::Config) -> Check {
    let (state, status, severity, detail) = match cfg.kanban.human_alert_channel_id.as_deref() {
        None => (
            ...
            CheckStatus::Warn,
            Severity::Warning,
            "No operational notification target declared in local config. The effective KV settings and notification delivery status of the running server were not checked.",
        ),
        Some(value) if value.trim().is_empty() => (
            "local_target_blank",
            CheckStatus::Warn,
            Severity::Warning,
            "The operational notification target in local config is empty. The effective KV settings and notification delivery status of the running server were not checked.",
        ),
        Some(_) => (
            "local_target_declared_unverified",
            CheckStatus::Pass,
            Severity::Info,
            "An operational notification target string is declared in local config. Target validity, effective KV reflection, permissions, and delivery were not verified.",
        ),
    };
    ...
    check.status = status;
    check.guidance = Some("When reviewing notification settings, check the effective kanban_human_alert_channel_id setting of the running server using a separate read-only procedure. This diagnostic does not perform notification activation or recovery. It does not suggest setting CIRCUIT_STAMP=1 as a solution.".into());
    check
}
