...
pub async fn status(state: State<AppState>) -> AppResult<(StatusCode, Json<Value>)> {
    service::status(&state).await
}

pub async fn draft_get(state: State<AppState>) -> AppResult<(StatusCode, Json<Value>)> {
    service::draft_get(&state).await
}

pub async fn draft_put(
    body: Json<service::OnboardingDraft>,
) -> AppResult<(StatusCode, Json<Value>)> {
    service::draft_put(body.0).await
}

pub async fn draft_delete() -> AppResult<(StatusCode, Json<Value>)> {
