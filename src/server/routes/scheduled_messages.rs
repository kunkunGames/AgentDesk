...
async fn validate_targeting(
    ...
    if target_channel_id.is_none() && bindings.primary_channel().is_none() {
        return Err(error_response(
            StatusCode::BAD_REQUEST,
            format!(
                "agent '{agent_id}' has no primary Discord channel; set targetChannelId explicitly"
            ),
        ));
    }
    Ok(())
...
async fn build_patch(
    ...
    let effective_scheduled_at = patch.scheduled_at.unwrap_or(existing.scheduled_at);
    ...
    if let Some(schedule) = effective_schedule.as_deref() {
        crate::services::routines::next_due_after(schedule, &effective_timezone, now)
