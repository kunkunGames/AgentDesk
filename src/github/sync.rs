...
async fn close_pg_card_for_issue(
    ...
    for intent in &decision.intents {
        execute_pg_transition_intent(&mut tx, intent).await?;
    }
    cancel_live_dispatches_for_terminal_card_pg(&mut tx, &card.id).await?;
    tx.commit()
        .await
...
pub(crate) async fn execute_pg_transition_intent(
    ...
    match intent {
        ...
        crate::engine::transition::TransitionIntent::SyncAutoQueue { card_id } => {
            sync_auto_queue_terminal_on_pg(tx, card_id).await?;
        }
        crate::engine::transition::TransitionIntent::SyncReviewState { card_id, state } => {
            sync_review_state_on_pg(tx, card_id, state).await?;
        }
