//! Agent JSON serialization helpers (#3037 bucket 4).
//!
//! These domain serialization functions (and their backing view structs) build
//! the `serde_json::Value` payloads consumed by the agent query/turn services.
//! They previously lived in `crate::server::dto::agents`, which forced the
//! services layer to reach back up into the server layer. They now live beside
//! their only callers in the services layer; the serde *response container*
//! types stay in `server::dto::agents` because they are presentation DTOs used
//! solely by the route handlers.
