...
pub(in crate::services::discord::turn_bridge) struct SavedExitCandidateProjection<'a> {
    pub(in crate::services::discord::turn_bridge) runtime:
        super::super::bridge_entry_persist::BridgeEntryRuntimeState<'a>,
    pub(in crate::services::discord::turn_bridge) last_edit_text: &'a mut String,
}

pub(in crate::services::discord::turn_bridge) fn reconcile_saved_exit_candidate(
    shared: &SharedData,
    state: &mut SavedExitCandidateProjection<'_>,
    ...
    super::super::bridge_entry_persist::clear_last_edit_text_if_current_message_changed(
        ...
        *state.runtime.current_msg_id,
        state.last_edit_text,
    );
}

fn saved_exit_candidate_projection<'a>(
    state: &'a mut StreamLoopState<'_>,
) -> SavedExitCandidateProjection<'a> {
    SavedExitCandidateProjection {
        runtime: super::super::bridge_entry_persist::BridgeEntryRuntimeState {
            ...
            bridge_confirmed_response_sent_offset: &mut *state
        ...
        last_edit_text: &mut *state.last_edit_text,
    }
}
...
pub(super) async fn settle_and_reconcile_exit_candidate<G: TurnGateway + ?Sized>(
    ...
    .await
    {
        let mut projection = saved_exit_candidate_projection(&mut context.state);
