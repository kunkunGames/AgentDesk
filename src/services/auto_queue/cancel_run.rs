...
impl AutoQueueService {
    ...
    pub fn cancel_run(
        ...
        match run_status.as_deref() {
            Some("active") | Some("paused") | Some(RUN_STATUS_RESTORING) => {
                Ok(cancel_selected_runs_with_conn(
                    health_registry,
                    conn,
                    &[run_id.to_string()],
                    "auto_queue_cancel",
    ...
    pub fn cancel_runs(
        ...
        Ok(cancel_with_conn(health_registry, conn))
    }
}

pub(crate) fn load_run_ids_with_status(
    conn: &libsql_rusqlite::Connection,
    statuses: &[&str],
...
pub(crate) fn load_live_dispatch_ids_for_runs(
    conn: &libsql_rusqlite::Connection,
    run_ids: &[String],
) -> libsql_rusqlite::Result<Vec<String>> {
    if run_ids.is_empty() {
...
pub(crate) fn cancel_live_dispatches_for_runs(
    conn: &libsql_rusqlite::Connection,
    run_ids: &[String],
    reason: &str,
) -> usize {
            ...
            for (bound_run_id, agent_id, slot_index) in bindings {
                *run_release_candidates.entry(bound_run_id).or_default() += 1;
                if released_slots.insert((agent_id.clone(), slot_index)) {
                    cleared_sessions += super::runtime::clear_slot_threads_for_slot(
...
pub(crate) fn cancel_selected_runs_with_conn(
    health_registry: Option<Arc<crate::services::discord::health::HealthRegistry>>,
    conn: &libsql_rusqlite::Connection,
    target_run_ids: &[String],
    reason: &str,
