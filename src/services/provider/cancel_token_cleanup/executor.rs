...
pub(crate) fn with_executor_dispatch_seam(test: impl FnOnce()) {
    ...
    let _lock = TEST_LOCK.get_or_init(|| Mutex::new(())).lock().unwrap_or_else(|error| error.into_inner());
...
mod tests {
    ...
    fn bind(token: &CancelToken, name: &str) {
        *token.tmux_binding.lock().unwrap_or_else(|error| error.into_inner()) = authority::publish(
            ProviderKind::Claude,
            name,
    ...
    fn unregistered_binding_dispatches_pid_and_tmux_cleanup() {
        with_seam(|| {
            let token = CancelToken::new();
            *token.tmux_binding.lock().unwrap_or_else(|error| error.into_inner()) = Some(TmuxBinding::NameOnly {
    ...
    fn stale_binding_is_retained_and_dispatches_no_primitive() {
        with_seam(|| {
            ...
            assert!(token.tmux_binding.lock().unwrap_or_else(|error| error.into_inner()).is_some());
            assert!(token.child_pid.lock().unwrap_or_else(|error| error.into_inner()).is_some());
        });
    }
}
