use anyhow::Result;
use rusqlite::{Connection, OptionalExtension, params_from_iter}; // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
...
fn auto_queue_has_column(conn: &Connection, table: &str, column: &str) -> bool {
    conn.query_row(
        ...
        rusqlite::params![table, column], // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
        |row| row.get(0),
    )
    .unwrap_or(false)
}
...
fn rebuild_auto_queue_phase_gates_table(conn: &Connection) -> Result<()> {
    ...
    let mut rows = conn
        ...
        .collect::<rusqlite::Result<Vec<_>>>()?; // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
...
fn backfill_auto_queue_dispatch_history(conn: &Connection) -> Result<()> {
    ...
    for dispatch in dispatches {
        ...
        conn.execute(
            ...
            rusqlite::params![entry_id, dispatch_id, created_at], // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
        )?;
    }
    ...
    for row in parsed_rows {
        ...
        if valid_dispatch_ids.is_empty() {
            conn.execute(
                ...
                rusqlite::params![ // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
                    row.run_id,
                    row.phase,
                    row.status,
                    row.verdict,
...
fn backfill_session_agent_ids(conn: &Connection) -> Result<()> {
    ...
    for (session_key, thread_channel_id, dispatch_id) in sessions {
        ...
        conn.execute(
            ...
            rusqlite::params![session_key, agent_id], // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
        )?;
    }
        ...
        for (card_id, legacy_status, existing_blocked_reason) in legacy_cards {
            let prior_status = conn
                .query_row(
                    ...
                    rusqlite::params![card_id, legacy_status], // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
                ...
                conn.execute(
                    "UPDATE kanban_cards
                     SET status = ?1,
                         ...
                         updated_at = datetime('now')
                     WHERE id = ?3",
                    rusqlite::params![target_status, blocked_reason, card_id], // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
                )?;
                conn.execute(
    ...
    for (id, session_key, dispatch_id) in transcripts {
        ...
        conn.execute(
            "UPDATE session_transcripts
             ...
             WHERE id = ?1
               AND NULLIF(TRIM(agent_id), '') IS NULL",
            rusqlite::params![id, agent_id], // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
        )?;
    }
    ...
    fn backfill_auto_queue_phase_gates_chunks_large_fk_lookups() {
        ...
        for index in 0..total {
            ...
            conn.execute(
                ...
                rusqlite::params![card_id, format!("Phase Gate Card {index}")], // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
            )
            .unwrap();
            conn.execute(
                "INSERT INTO auto_queue_runs (id, repo, agent_id, status)
                 VALUES (?1, 'test/repo', 'agent-1', 'paused')",
                rusqlite::params![run_id], // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
            )
            .unwrap();
            conn.execute(
                "INSERT INTO task_dispatches (
                ...
                rusqlite::params![dispatch_id, card_id, format!("Phase Gate Dispatch {index}")], // TODO(#839): sqlite compatibility retained for out-of-scope callers or legacy tests.
            )
            .unwrap();
            conn.execute(
