...
pub fn upsert_turn_owned_sqlite(db: &crate::db::Db, entry: &PersistTurnOwned) -> Result<()> {
    ...
    let conn = db
        .lock()
        .map_err(|error| anyhow!("open sqlite turn persistence connection: {error}"))?;
    conn.execute(
        "INSERT INTO turns (
            turn_id,
            session_key,
    ...
    .map_err(|error| anyhow!("persist sqlite turn failed: {error}"))?;
