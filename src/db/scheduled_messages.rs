...
async fn arm_delivery_slot_tx(
    ...
    let armed = sqlx::query(
        "INSERT INTO scheduled_message_deliveries
         ...
         VALUES ($1, $2, $3, $4, 'running', $5, $6 + ($7::bigint * INTERVAL '1 second'))
    ...
    let updated = sqlx::query(
        "UPDATE scheduled_message_deliveries
         SET status = $2, error = $3,
             ...
             fallback_outbox_id = COALESCE($5, fallback_outbox_id),
             finished_at = NOW(), updated_at = NOW()
         WHERE id = $1 AND status = 'running'",
    )
    .bind(delivery_id)
...
pub async fn list_running_agent_deliveries_pg(
    ...
    sqlx::query_as::<_, RunningAgentDelivery>(
        "UPDATE scheduled_message_deliveries d
         ...
         RETURNING d.id AS delivery_id, d.scheduled_message_id, d.turn_id,
                   d.started_at,
...
pub async fn recover_expired_leases_pg(pool: &PgPool) -> Result<u64, sqlx::Error> {
    ...
    let rows = sqlx::query(
        "UPDATE scheduled_message_deliveries
         ...
         RETURNING id, scheduled_message_id, fire_scheduled_at",
    )
    .fetch_all(&mut *tx)
    .await?;
    ...
    for row in rows {
        ...
        // Only rewind parents that still point at the interrupted run
        // (routines boot-recovery principle), restoring the original fire slot
        // so the re-arm hits the same uq_smdel_fire_slot row.
        sqlx::query(
            "UPDATE scheduled_messages
             SET status = 'scheduled', scheduled_at = $3,
