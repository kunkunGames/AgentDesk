...
mod tests {
    ...
    #[test]
    fn test_scrub_worktree_keys_from_json_scrubs_keys() {
        let raw = Some(r#"{"keep": "value", "remove1": "bad", "remove2": "worse"}"#);
        let keys = &["remove1", "remove2"];
        let result = scrub_worktree_keys_from_json(raw, keys);
        assert_eq!(result, Some(r#"{"keep":"value"}"#.to_string()));
    }

    #[test]
    fn test_scrub_worktree_keys_from_json_no_rewrite_needed() {
        let raw = Some(r#"{"keep": "value"}"#);
        let keys = &["remove1"];
        let result = scrub_worktree_keys_from_json(raw, keys);
    ...
    #[test]
    fn test_scrub_worktree_keys_from_json_non_object() {
        let raw = Some(r#"["keep", "value"]"#);
        let keys = &["remove1"];
        let result = scrub_worktree_keys_from_json(raw, keys);
        assert_eq!(result, None);
    ...
    #[test]
    fn test_scrub_worktree_keys_from_json_invalid_json() {
        let raw = Some(r#"{"keep": "value""#);
        let keys = &["remove1"];
        let result = scrub_worktree_keys_from_json(raw, keys);
        assert_eq!(result, None);
    ...
    #[test]
    fn test_scrub_worktree_keys_from_json_none() {
        let result = scrub_worktree_keys_from_json(None, &["remove1"]);
        assert_eq!(result, None);
    }

    #[test]
    fn test_scrub_worktree_keys_from_json_empty_string() {
        let result = scrub_worktree_keys_from_json(Some(""), &["remove1"]);
        assert_eq!(result, None);
    }
}
