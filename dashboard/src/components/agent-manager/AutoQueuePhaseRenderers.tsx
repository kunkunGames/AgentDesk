...
export function createAutoQueuePhaseRenderers(ctx: AutoQueuePhaseRendererCtx) {
  ...
  // The deploy variant follows the phase's gate kind (`gateKindByPhase`);
  // `gatesByPhase` supplies the gate row's status and failure reason.
  const renderPhaseGateIndicator = (phase: number) => {
    const gates = gatesByPhase.get(phase) ?? [];
    const isDeploy = gateKindByPhase.get(phase) === DEPLOY_GATE_KIND;
