...
| Path | Purpose |
| --- | --- |
| `src/main.rs` | Entry point. Dispatches CLI commands or boots the server runtime. |
| `src/bootstrap.rs` / `src/launch.rs` | Bootstrap config, DB, shared state, then launch the app. |
| `src/cli/` | Operator-facing CLI, direct API shims, migration tools, Discord send helpers. |
| `src/config.rs` | `agentdesk.yaml` parsing and configuration defaults. |
| `src/db/` | SQLite schema and query helpers. `src/db/schema.rs` is the schema authority. |
| `src/dispatch/` | Dispatch context construction, review context, worktree targeting. |
| `src/engine/` | QuickJS policy runtime, hook wiring, transition logic, Rust↔JS bridge. |
| `src/github/` | GitHub sync, triage, Definition-of-Done mirroring. |
| `src/server/` | Axum server boot, routes, workers, background loops, WebSocket broadcast. |
| `src/services/` | Core runtime services: providers, Discord bot, queueing, memory, platform helpers. |
| `src/runtime_layout.rs` / `src/runtime.rs` | Runtime directory layout and persistent runtime state helpers. |
| `src/kanban.rs` / `src/pipeline.rs` / `src/reconcile.rs` | Kanban orchestration, pipeline helpers, boot-time reconciliation. |
...
- `python3 scripts/generate_inventory_docs.py --check` is the CI drift gate for these inventories and the generated `src/` snapshot above.
