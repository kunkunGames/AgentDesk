...
# database coordinate through runner heartbeats, advisory hub leases,
...
The offline session-name command is reproducible for a fixed configuration: pin `AGENTDESK_CONFIG` when running it from different directories, or pass `--provider` to bypass inference. Without the flag it validates the whole selected config and reports errors, including errors outside `onboarding`; it does not silently route with defaults for an invalid file. Other legacy CLI commands using the graceful loader retain their existing warning-and-fallback behavior on parse errors. `onboarding.provider_options` is a reserved compatibility field with no runtime effect; dashboard picker aliases remain separate from backend routing. Legacy tmux session names without an explicit provider prefix keep their historical registry provider regardless of `default_provider` changes.
