...
while IFS= read -r scratch_path; do
  ...
  FAIL=1
done < <(find . \( -path "./target" -o -path "./.git" -o -path "*/node_modules" \) -prune -o -type f \( -name "plan.md" -o -name "pr-body.md" -o -name "pr_body.md" -o -name "scratch.md" -o -name "scratch.txt" -o -name "scratchpad.md" -o -name "scratchpad.txt" -o -name "test_scratch.rs" -o -name "plan.txt" \) -print)
if [ "$FAIL" -ne 0 ]; then
  exit "$FAIL"
fi
