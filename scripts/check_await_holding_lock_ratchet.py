...
# (src/services/discord/tui_direct_pending_start/tests.rs) hold `runner_test_lock()`
# across `tokio::time::advance` awaits that drive `run_runner`. The guard
...
BASELINE = 55  # -1 (#4833): escalation env isolation now uses the canonical re-entry-checked shared test-env guard, removing its await_holding_lock suppression. # +1 (#4795): busy-retry FIFO regression test holds shared_test_env_lock across await to serialize AGENTDESK_ROOT_DIR (RuntimeRootGuard tempdir) against parallel tests (test-only, SAFETY comment at site). # +16 (#4091 r7 batch-4): crate-wide test env-lock isolation sweep — async tests in queue_io/race_loss/terminal_controller_cutover/voice-pcm-harness/operator_connectors/worktree_orphan_sweep now hold shared_test_env_lock across await to serialize AGENTDESK_ROOT_DIR against parallel tests (all test-only, SAFETY-commented). # +1 (#4091 r5): tui_prompt_relay runtime-binding test holds tui_prompt_dedupe::TEST_LOCK across await to serialize shared dedupe-map resets against parallel tests (test-only, SAFETY comment at site). # +1 (#4068): mailbox snapshot peek-only test holds lock_test_env() guard across await to serialize AGENTDESK_ROOT_DIR. # +1 (#4979 S2 v2): setup-lock cleanup wrapper is a test-only serialization site; its narrow SAFETY allow is required by the crate-wide deny. # +1 (#5071 relay-tail S2): the r4 call-site witness test `call_site_withholds_the_pane_idle_witness_for_an_unmeasured_tail` (src/services/discord/health/snapshot.rs) holds lock_test_env() across the build_health_snapshot awaits to serialize AGENTDESK_ROOT_DIR (tempdir runtime root) against parallel tests — the same test-only serialization shape as the #4068 site in this module; releasing it before the awaits would let concurrent tests stomp the shared env.
