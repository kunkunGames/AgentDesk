...
def main():
    ...
    for pr in prs:
        ...
        if not has_non_empty_body_field(body, ["verification", "verification commands and results"]):
            print("  [!] MISSING VERIFICATION: PR body lacks the required 'verification' commands and results.")
        if not has_non_empty_body_field(body, ["skipped checks", "skipped checks with reasons"]):
