...
def is_scratch_file_path(path):
    ...
    if is_root:
        ...
        if filename in root_scratch_files:
            return True

        return bool(
            re.match(r"^(?:scratch|scratchpad|test_scratch)(?:[._-].+)?\.(?:md|txt|sh|sql|rs|py|js|json)$", filename)
            or re.match(r"^test_[A-Za-z0-9._-]+\.(?:rs|py|js|json)$", filename)
        )
    return False
