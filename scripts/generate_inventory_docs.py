...
def render_module_inventory(entries: list[ModuleEntry]) -> str:
    production_entries = [entry for entry in entries if entry.prod_line_count > 0]
    namespace_counts = Counter(
        entry.module_path.split("::", 1)[0] for entry in production_entries
    )
    ...
    lines = [
        ...
        "",
        f"- Production Rust modules: `{len(production_entries)}`",
        f"- Giant-file threshold: `>= {GIANT_FILE_THRESHOLD}` production lines",
