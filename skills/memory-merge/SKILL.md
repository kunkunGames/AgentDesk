...
- `sink`: `backend` 해석 결과에 따라 `file | memento | mem0`
- `backend=auto`면 `memento -> mem0 -> file` 순서로 감지
- 명시 지정(`memento`, `mem0`, `file`)이면 자동감지 skip
...
4. **Phase 4: sink backend 결정** — `file | memento | mem0`
5. **Phase 5: backend별 기록**
   ...
   - `mem0`: preference / relation만 Mem0 MCP에 기록, 나머지는 skip
...
- `memento`/`mem0` backend에서 현재 턴에 MCP를 쓸 수 없으면 file로 몰래 바꾸지 말고 `skip + 이유`를 보고한다
