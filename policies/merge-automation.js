...
function inspectLatestCompletedWorkTarget(cardId) {
  ...
  for (var i = 0; i < excludedRows.length; i++) {
    ...
    var canFallback =
      !!excludedTarget.worktree_path || !!excludedTarget.branch || !!excludedTarget.head_sha;
    inspected.push({
    ...
    if (!fallbackTarget && canFallback) {
      fallbackTarget = excludedTarget;
      fallbackIndex = inspected.length - 1;
    }
  }
  ...
  if (fallbackTarget) {
    inspected[fallbackIndex].selected = true;
    inspected[fallbackIndex].reason =
      "selected latest non-completed work dispatch as fallback candidate";
    return { target: fallbackTarget, inspected: inspected };
  }
