...
MCP-backed Memento data is stored under the operator-private MCP server and is accessed through the `mcp__memento__*` tool surface. `/api/memory/*` is route-level dual-mode only where a bridge exists; `/api/memory/recall` and `/api/memory/forget` must not be treated as full MCP-backed Memento access until those bridges are implemented.
...
Audit fallback memory separately from MCP-backed memory. Query the table directly with `psql "$DATABASE_URL" -c "SELECT id, topic, kind, workspace, created_at FROM local_memory ORDER BY created_at DESC LIMIT 20;"` or call `curl -sf -X POST http://127.0.0.1:8791/api/memory/recall -H 'Content-Type: application/json' -d '{"limit":20}'` and verify the response reports `source: "local"` while recording `detected_backend`. Because MCP recall/forget bridges are still incomplete, a clean fallback recall result only clears the fallback table, not the MCP-backed Memento store.
