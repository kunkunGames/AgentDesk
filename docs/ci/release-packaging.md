...
같은 OS/architecture의 허브와 실행 노드는 같은 실행 파일과 공통 자산을 사용한다.
역할은 노드 설정과 런타임 시작 계획으로 정한다. 실행 노드 수나 역할을 빌드 matrix에
...
run `35651612825`에서 모두 성공했다. 이후 실행 노드 기능의 최종 commit은 별도 검증한다.

현 단계에서는 OS/CPU별 공통 바이너리와 런타임 역할 선택을 유지한다. 실행 노드의 UI 자산
설치 생략이 별도 실행 노드 프로그램을 만드는 것보다 작은 변경으로 배포량을 줄인다.
