...
| 기존 컴포넌트 | 역할 재사용 |
|---|---|
| `message_outbox` (0001, 0042, 0066) | 최종 Discord 전송 큐. push 모드 발화 시 여기로 enqueue. **outbox drain claim이 이미 `next_attempt_at <= NOW()`를 게이트하므로(`server/mod.rs` claim_pending_message_outbox_batch_pg) 지연 전송·재시도·claim을 outbox가 온전히 소유** — handoff 이후 스케줄러는 관여하지 않는다. slot dedupe sentinel(`dedupe_key` 있음 + `dedupe_expires_at` NULL)은 server GC와 maintenance retention에서 제외한다 |
| headless agent turn (`services/discord/health`) | agent 모드에서 턴 ID intent를 먼저 저장하고 `start_reserved_headless_agent_turn_with_owner_channel`가 `started`를 반환한 뒤 launch를 확정해 대상 채널에 relay. 완료는 routines와 같은 transcript/quality-event 증거 모델로 판정 |
...
`migrations/postgres/0082_scheduled_messages.sql`부터 `0084_scheduled_message_delivery_handoff_state.sql`까지
(`0079`~`0081`은 최신 upstream 계열이 선점).
...
CREATE TABLE IF NOT EXISTS scheduled_message_deliveries (
    ...
    resume_scheduled_at TIMESTAMPTZ,                -- trigger-now 뒤 반복을 재개할 원래 slot
    ...
    -- agent 경로 추적
    turn_id             TEXT,                       -- 예약한 에이전트 턴 ID (launch intent)
    turn_started_at     TIMESTAMPTZ,                -- runtime이 실제 started를 확인한 시각
    fallback_outbox_id  BIGINT,                     -- on_agent_failure='push_raw' 강등 시
    retry_count         INTEGER NOT NULL DEFAULT 0,
    next_attempt_at     TIMESTAMPTZ,                 -- 예약된 재시도 필드(현재 re-arm은 NULL로 clear)
    error               TEXT,
...
- **agent 모드의 "sent" 판정 = relay 증거 확인**: 예약한 turn ID의
  ...
  성공이 아니며, `empty_response` turn error와 30분 내 transcript 미생성도
  `on_agent_failure` 정책으로 처리한다. 별도 ack API는 구현되어 있지 않다.
...
- **만료는 실제 발화 시각에도 재검사**하며 retry 소진 처리보다 우선한다. due claim 후 각
  delivery의 handoff 직전 fresh clock에서 `expires_at`이 지났으면 이미 생성한 delivery를 `interrupted`(사유: definition expired)로
...
1. due-claim (다중 노드 안전):
   ...
   Discord runtime이 없는 프로세스에서는 push/agent 모두 claim하지 않는다.
   push도 같은 프로세스의 `message_outbox_loop`가 있어야 실제 전송되기 때문이다.
   → 첫 attempt: delivery row INSERT, claim_owner/claim_token/2분 lease 기록.
   → 중단된 같은 slot: ON CONFLICT ... WHERE status='interrupted'로 같은
     delivery row를 즉시 re-arm. claim_token 교체, retry_count + 1,
...
2. 발화:
   ...
   - push:  message_outbox INSERT
            ...
            slot 단위의 만료 없는 durable dedupe key; retention/GC 제외)
            → 부모→delivery claim lock을 유지한 같은 transaction에서 outbox row ID를
              delivery에 기록한다. 취소가 먼저 이 lock을 얻으면 handoff를 건너뛴다.
            → 새 row 또는 기존 활성 row ID를 확보하면 즉시 delivery=sent
              (터미널, "handed off").
              이후 재시도/실패는 outbox_loop 소유 — 여기서 다시 감시하지 않는다.
   - agent: primary channel/provider binding을 해석하고 target(미지정 시 primary)에서
            headless turn을 시작. 부모→delivery claim lock과 claim_token 조건으로
            예약한 turn ID를 launch intent로 먼저 기록하되 lease는 갱신하지 않는다.
            외부 start 직전에 parent/claim/turn intent를 다시 확인해 그 사이 취소가
            이겼다면 turn을 시작하지 않는다.
            runtime이 `started`를 반환한 뒤에만 `turn_started_at`과 lease를 갱신한다.
            Discord runtime이 없거나 provider runtime이 아직 부팅 중이면 delivery
            slot을 제거하고 부모를 원래 slot으로 되돌려 retry_count를 소모하지 않는다.
            assistant reply는 target 채널에 그대로 relay된다.

3. 완료 감시 (agent 모드 running delivery만 해당):
   - `turn_started_at`이 있는 turn ID의 non-empty assistant transcript → sent.
   - NO_REPLY, empty_response, 30분 timeout → push_raw fallback handoff 후 sent,
     또는 fail 정책에 따라 failed.
   - 폴링 대상의 2분 lease를 갱신. 현재 attempt 워커의
     완료/실패/중단 쓰기는 claim_token과 running 상태를 모두 만족해야 한다.

4. 재시도/부모 갱신:
   - lease 만료 또는 transient 실패 → 부모를 먼저 lock한 뒤 delivery를
     interrupted로 바꾸고, 부모를 같은 fire_scheduled_at의 scheduled로 rewind.
     다음 tick이 같은 row를 즉시 fenced re-arm하며 별도 backoff row/scan은 없다.
     3회 re-arm 예산을 소진한 다음 claim은 터미널 실패(또는 push_raw)로 닫는다.
     push_raw outbox enqueue 자체가 일시 실패하면 현재 slot을 interrupted/re-arm해
     fallback을 유실하지 않는다. push_raw handoff가 성공하면 현재 delivery는 sent이지만, 재시도 소진은
...
확인한다. turn 시작/폴러가 lease를 선점 갱신했다면 복구는 아무것도
바꾸지 않는다. 이 복구는 워커의 매 tick에 실행되어 프로세스 재시작과
리더 교체 모두를 다룬다. `turn_id`만 있고 `turn_started_at`이 없는 launch
intent는 폴링/lease 갱신 대상이 아니므로 시작 전 크래시는 정상적으로 복구된다.
...
- `deliveryKind='agent'` && 해당 agent의 primary Discord channel 없음 →
  `targetChannelId` 명시 여부와 무관하게 거부. primary channel은 headless turn의
  owner/session 컨텍스트이고, `targetChannelId`는 relay 대상이다.
- agent의 명시적 `targetChannelId`가 등록 alias도 숫자 Discord channel ID도
  아니면 fire-time retry로 넘기지 않고 생성/PATCH 시점에 거부한다.
...
과거면 PATCH도 현재 시각 이후의 다음 schedule 시각으로 보정한다. PATCH가
`schedule`만 제거해 효과적인 정의가 과거 one-shot이 되는 경우도 거부한다.

### DELETE `/api/scheduled-messages/:id` — 취소

- `scheduled` → `canceled` (200)
- `firing` → 부모→delivery lock 순서로 현재 claim과 직렬화한 뒤 진행 delivery를
  `interrupted`로 마킹. handoff 전 취소가 먼저 lock을 얻으면 outbox/turn 시작은
  차단된다. 이미 handoff ID가 기록된 뒤면 전송 자체는 막지 못함 →
  `{"canceled": true, "note": "delivery was already handed off; downstream delivery may still complete"}`
- 터미널 상태 → 409
...
`scheduledAt`이 미래면 즉시 발화 완료 후에도 그 미래 slot을 유지한다.
Discord delivery runtime이 없는 프로세스에서는 push/agent 모두 503으로 거부한다.
...
3. **반복은 선택 기능이고 slot 기준으로 anchor**: 1회성 예약이 1급
   ...
   trigger-now는 `resume_scheduled_at`에 원래 미래 slot을 보존하므로 중단/re-arm
   뒤에도 수동 발화 시각으로 반복 기준이 이동하지 않는다.
...
| 파일 | 내용 |
|---|---|
| `migrations/postgres/0082_scheduled_messages.sql` ~ `0084_scheduled_message_delivery_handoff_state.sql` | 위 스키마 + info-only bot 기본값 + recurrence/turn launch 상태 보정 (+ immutable-checksums.json 갱신) |
| `src/db/scheduled_messages.rs`, `src/db/scheduled_messages/{handoff,agent_polling}.rs` | CRUD + due-claim + delivery 상태 전이/transaction handoff + agent poll/lease recovery 쿼리 |
...
| `src/server/outbox_gc.rs`, `src/services/maintenance/jobs/db_retention.rs` | 영구 outbox dedupe sentinel을 두 retention 경로에서 보존 |
