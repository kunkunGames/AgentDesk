...
2. 발화:
   ...
   - agent: primary channel/provider binding을 해석하고 target(미지정 시 primary)에서
            headless turn을 시작. 예약한 turn ID를 claim_token 조건으로
            delivery에 먼저 기록하고 lease를 갱신한 뒤 실제 turn을 시작.
...
3. 완료 감시 (agent 모드 running delivery만 해당):
   ...
   - lease 만료가 가까운 순서로 batch를 순환하며 폴링 대상의 2분 lease를 갱신.
     현재 attempt 워커의
     완료/실패/중단 쓰기는 claim_token과 running 상태를 모두 만족해야 한다.

4. 재시도/부모 갱신:
   - turn ID 기록 전 lease 만료 또는 transient 실패 → 부모를 먼저 lock한 뒤 delivery를
...
확인한다. turn ID가 아직 없는 pre-turn row만 re-arm한다. 이미 turn ID가
기록된 row는 새 turn을 만들지 않고 다음 leader의 poller가 같은 durable
turn을 adopt해 lease를 갱신한다. 이 구분은 claim token으로 막을 수 없는
...
과거면 PATCH도 현재 시각 이후의 다음 schedule 시각으로 보정한다.
