...
- **만료는 실제 발화 시각에도 재검사**하며 retry 소진 처리보다 우선한다. due claim 후 `expires_at <=
...
1. due-claim (다중 노드 안전):
   ...
   Discord runtime이 없는 프로세스에서는 agent 정의를 claim 대상에서 제외한다.
   ...
   → 중단된 같은 slot: ON CONFLICT ... WHERE status='interrupted'로 같은
     ...
     outbox_id/turn_id/fallback_outbox_id/error/완료 시각을 초기화.
...
2. 발화:
   ...
   - agent: primary channel/provider binding을 해석하고 target(미지정 시 primary)에서
            ...
            예약한 turn ID를 delivery에 먼저 기록하고 lease를 갱신한 뒤 실제 turn을 시작.
            Discord runtime이 없으면 delivery slot을 제거하고 부모를 원래 slot으로
            되돌려 retry_count를 소모하지 않는다.
