...
Discord runtime 자체가 없는 프로세스는 agent 정의를 claim하지 않으며, direct
fire 경로도 retry budget을 소비하지 않고 정의를 되돌린다.
...
agent 전달인데 이 프로세스에 Discord runtime이 없으면 claim 전에 503을 반환한다.
