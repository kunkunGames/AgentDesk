...
- **B3 demote memory lease to a cache + enforce the guard** — promote the #3154 observe-only monotonic guard to **enforce** now that a single authority exists (closes #3416). **AC6 met here.**
  - **As-built (B3):** the enforce is `validate_inflight_state_for_save` returning a verdict the plain-overwrite save tails (`save_inflight_state_in_root`, `persist_under_lock`) honor by skipping a same-turn BACKWARD offset write (caller keeps the higher on-disk state). Gated by the SAME `AGENTDESK_DELIVERY_RECORD_AUTHORITY` flag as the B2 read-flip via the pure `delivery_record::authority_blocks_backward_inflight_write` helper, so the single authority + its enforcement cut over atomically; flag OFF → byte-identical observe-only. That was the compiled default through #5071 T1 S8-1r2 (making the slice a deploy no-op); since #5071 T1 S8-2 the compiled default is ON and OFF is reached only by an explicit `=0`. The debug-build `debug_assert` still fires first in debug; enforce is the release-build graceful skip.
  ...
  - **As-built compiled-default promotion (#5071 T1 S8-2):** `resolve_authority_flag` stops delegating to the shared opt-in reading and resolves absence to `compiled_default(true)`, so the durable `delivered_frontier` is the repo-owned dedup authority on an unpinned node (#5262). Present-value parsing is untouched — still `EnvOverride` for anything present, ON only for `1`/`true` — so `=0` becomes the rollback lever. Pure default transition: no read path, write path, guard, or health-field shape changed. The shadow axis keeps its compiled default OFF, so an unpinned node now reports `mode: authority_only` with the single `delivery_record_shadow_disabled` warning.
  - **Still deferred to post-canary:** deprecating the legacy duplicate markers behind a version bump — the legacy markers remain the fallback until the ON default has been validated in production across both nodes. The "flag still default OFF" premise this bullet originally rested on expired with the S8-2 flip; the gate is now the two-node comparison in `docs/agent-maintenance/multinode-transition.md`, not the compiled default.
