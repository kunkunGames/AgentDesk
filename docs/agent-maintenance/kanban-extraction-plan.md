...
| `kanban::github_sync` | `src/kanban.rs:1214` through `src/kanban.rs:1302`, `src/kanban.rs:1840` through `src/kanban.rs:1976` | 95 | 140 | GitHub sync trigger and target lookup plus legacy test-cfg audit logging helpers. | `kanban::transition`, GitHub sync runner, audit tests. |
