...
- do_not_edit_without_migration_plan:
  ...
  - `src/services/message_outbox.rs` is the PG-backed message outbox
    ...
    state that runners cannot claim; callers activate it to `pending` only
...
- invariants: every new `send` or `edit` from production code goes through
  ...
  runner. Interaction-token responses (`ctx.say`, `ComponentInteraction`) are
...
- do_not_edit_without_migration_plan (giant-file):
  ...
  - `src/services/discord/tmux_watcher.rs` (frozen giant surface; #4229 W7b S-A moved the terminal token update and disabled auto-compact tail verbatim to the non-giant `tmux_watcher/terminal_token_update.rs` child module, ratcheting the root down after behavior-preserving decompose; #4895 removes the obsolete auth/overload prose-authority fields from the thin collector→exit wiring; untyped prose now remains a bounded diagnostic and cannot reach provider-clear/tmux-kill/retry execution; #4799 adds thin suppressed-terminal wiring only: footer-owned background completions enqueue one semantic-event-keyed lifecycle marker, while card-owned subagents are deliberately excluded to prevent a duplicate card+marker surface; #4229 S4
    ...
    the runner's OWN synthetic turn for #3303 `DeferredClaim` markers), so an
  ...
  - `src/services/discord/tui_prompt_relay.rs` (no longer a registered giant as
    ...
    runner-local relay lifecycle only, no PG lease/schema. The parent
  ...
  - `src/services/discord/tui_prompt_relay/synthetic_start.rs` (frozen giant surface;
    ...
    identity — the live row at the record instant, or the runner's LAST-VIEW
    ...
    via `tokio::task::spawn_blocking` so it never stalls a Tokio executor runner);
    ...
    re-records the lease as the watcher owner) — so the deferred runner is the
    ...
    observer stand down (deferred) AND no runner bridge tail AND background idle
    ...
    (the claim runs later in the runner), so the runner (`pending_start_claim_fn`)
    ...
    the BridgeAdapter) makes the runner spawn EXACTLY ONE bridge tail for the
    ...
    +71 from #3154 codex P1 (timestamp-anchor output loss): the runner-spawned
    ...
    inflight) on `TuiDirectSyntheticTurnClaim`, and the runner passes it as the
    ...
    +54 from #3282: the deferred pending-start runner's terminal backstop ABORT
  ...
  - `src/services/discord/tui_direct_pending_start{.rs,/state.rs}` (933 / 877
    ...
    flows, the runner FSM, marker wrappers, and no-evict post-abort promotion;
  ...
  - `src/services/discord/idle_recap/scrollback.rs` (#3479 scrollback: the tmux
    ...
    / `parse_transcript_line_text` runners), and the Haiku `compose_with_haiku`
    ...
    byte-identical `idle_recap::<fn>` call sites, while the parsing runners stay
  ...
  - `src/services/discord/terminal_ui_obligation.rs` (545 prod LoC; #3607
    runner-local durable terminal-UI obligation sidecar store plus isolated
  ...
  - `src/services/discord/runtime_bootstrap.rs` (285 production lines after #3479 item-3 grouped the 20 builder args into 6 param structs; was 274 after
    ...
    side-effect-order doc), and `gateway_runtime.rs` (147, the hub runtime
...
- do_not_edit_without_migration_plan (giant-file):
  ...
  - `src/cli/migrate/apply.rs` (frozen giant surface; +1 from #3690 AgentDef preferred_intake_node_labels literal; +6 from #3697 OpenClaw --write-db non-hub roster-sync gate).
...
- canonical_modules: `src/config.rs`, `src/runtime_layout/mod.rs`,
  ...
  (cluster role/hub-failover coordination), `src/server/runner_registry.rs`
  (supervised-runner registry / hub-only lifecycle), and
  `src/server/runner_recovery.rs` (runner-local restart budget/backoff execution).
...
- `src/services/discord/mod.rs` (frozen giant surface; formerly 4152 prod LoC after #4049 S4-a2 moved
  ...
  `src/services/discord_config_audit.rs` (frozen giant surface; +15 from #3692 hub-ownership gate on the config-audit agent sync path).
...
- `src/services/discord/standby_relay.rs` (frozen giant surface, giant-file;
  1012 production LoC) — the gateway-less runner standby relay. #4765 crossed
