...
- `--write-db`는 config-to-DB agent roster sync를 수행하므로 단독 운영 또는 명시적으로 `cluster.role: hub`인 노드에서만 실행합니다. runner/auto 노드에서는 안전하게 거부됩니다. 이전 역할 이름은 [호환 별칭](operations/node-terminology.md)으로 지원합니다.
