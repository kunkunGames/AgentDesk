...
| `src/services/message_outbox.rs:141-190,236-277` | `hardened` | PG-only authority is now explicit in code comments: lifecycle/outbox rows must land in PG because the release runner drains PG exclusively; retired SQLite-era fallbacks are not live runtime paths. |
