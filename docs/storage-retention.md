...
| PG `prompt_manifest_layers`               | 30 days                                     | `UPDATE full_content = NULL` (preserves hashes)              | `storage.prompt_manifest_retention`| Daily    |
...
Also `src/server/maintenance.rs` for `prompt_manifest_retention` job, and `src/server/routes/prompt_manifest_retention.rs` for retention status API.
