...
- [ ] **Infrastructure CI failure guard:** I have verified this PR does not attempt to "fix" a transient infrastructure CI failure (e.g., runner cancellation). Such issues must result in a no-change report, not a PR.
...
- [ ] **Unrelated change & scratch file audit:** I have run `git status` or a changed-file audit to ensure no ad-hoc scratch files (e.g. `plan.md`, `pr-body.md`) or unrelated test scripts (e.g. `.sh`, `.sql`) are included. If the diff contains unrelated files, old merged work, or broad stale branch baggage, I will abandon the branch and report that a clean PR is required.
