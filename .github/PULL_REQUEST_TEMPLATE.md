...
- [ ] **Duplicate PR guard:** I have checked for overlapping open PRs before creating this PR (especially for generated refresh work). If the `gh` CLI is unavailable, I have used `git fetch --all && git branch -r` as a fallback. If overlap is found, I have ended the run with a no-change report rather than opening an empty PR.
