...
- [ ] **Duplicate PR guard:** I have checked for overlapping open PRs using `gh pr list` or by inspecting remotes before creating this PR (especially for generated refresh work).
- [ ] **No-change verification:** If this PR claims no change, I have verified it modifies zero files using `gh pr view --json files` and that it explicitly changes a queue-hygiene artifact to justify being a PR instead of a report. (If an unavoidable no-change PR is opened, its body lists the exact overlapping PR numbers and branches).
...
- [ ] **Scratch file cleanup:** I have run `git status` or a changed-file audit to ensure no ad-hoc scratch files (e.g. `plan.md`, `pr-body.md`), unrelated test scripts (e.g. `.sh`, `.sql`), old merged work, or broad stale branch baggage are included in this PR.
