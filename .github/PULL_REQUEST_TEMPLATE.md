...
- [ ] **No-change verification:** If this PR claims no change, I have verified it modifies zero files using `gh pr view --json files` (running `git reset --hard` before pre-commit verifications is recommended to guarantee this). (If an unavoidable no-change PR is opened, its body lists the exact overlapping PR numbers and branches).
- [ ] **Merge readiness state:** I am not claiming merge-ready from partial check status; any pending or UNKNOWN mergeability is explicitly called out.
